...
class TrainGAN(nn.Module):
    ...
    def train_step(self, x):
        ...
        generator_outputs = self.generator.forward(z, max_len=100, temperature=1.0)
    ...
    def create_dataloader(self, data, batch_size=128, shuffle=True, num_workers=5):
        ...
        """
        print("tokens: ", self.tokenizer.test_char)
        print("vocab_size: ", self.tokenizer.vocab_size)
        print("start: ", self.tokenizer.start_token)
        print("end: ", self.tokenizer.end_token)
        return DataLoader(
            data,
    ...
    def save_checkpoint(self, step):
        ...
        torch.save(self.generator.state_dict(), os.path.join(save_path, "generator.pth"))
        torch.save(self.discriminator.state_dict(), os.path.join(save_path, "discriminator.pth"))

        # Save tokenizer state
        tokenizer_state = {
            'mapping': self.tokenizer.mapping,
            'inv_mapping': self.tokenizer.inv_mapping,
            'start_token': self.tokenizer.start_token,
            'end_token': self.tokenizer.end_token,
            'vocab_size': self.tokenizer.vocab_size
        }
        torch.save(tokenizer_state, os.path.join(save_path, "tokenizer.pth"))

        # Save generator optimizer state
        torch.save(self.generator_optim.state_dict(), os.path.join(save_path, "generator_optimizer.pth"))

        # Save discriminator optimizer state
        torch.save(self.discriminator_optim.state_dict(), os.path.join(save_path, "discriminator_optimizer.pth"))
    ...
    def train_n_steps(self, train_loader, max_step=10000, evaluate_every=50, save_every=20000):
        ...
        Args:
            ...
            save_every (int, optional): Defaults to 20000.

        Returns:
            float: Evaluation score after training
        """
        ...
        for step in range(140000, max_step): # range(max_step)
            try:
        ...
        return score  # Return the final evaluation score after training
